...
def donuts(count):
    # +++ ваш код +++
    if count >= 10:
        return("Кількість пончиків: багато")
    else:
        return("Кількість пончиків: "+str(count))
...
def not_bad(s):
    # +++ ваш код +++
    no = s.find('не',0,len(s))
    bad = s.find('погана',0,len(s))
    if no < bad:
        bad_end = bad + len('погана')
        r = s.replace(s[no:bad_end],'хороша')
    else:
        r = s
    return r
